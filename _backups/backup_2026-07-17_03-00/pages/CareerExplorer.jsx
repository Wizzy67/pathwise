...
const CareerExplorer = () => {
  ...
  return (
    <div className="min-h-screen bg-pw-black px-4 py-8 max-w-7xl mx-auto">

      {/* Heading */}
      <div className="mb-8">
        <h1 className="text-4xl md:text-5xl font-extrabold text-pw-white mb-2">
          Explore All Careers
          ...
          {!loading && (
            <div className="flex flex-wrap gap-2 mb-6">
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${!activeField ? 'border-pw-blue bg-pw-blue/10 text-pw-blue' : 'border-pw-white/15 bg-pw-white/5 text-pw-gray/90 hover:border-pw-blue hover:text-pw-blue'}`}
              >
                All
              </button>
              {fields.map(f => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${activeField === f ? 'border-pw-blue bg-pw-blue/10 text-pw-blue' : 'border-pw-white/15 bg-pw-white/5 text-pw-gray/90 hover:border-pw-blue hover:text-pw-blue'}`}
                >
                  {f}
                </button>
              ))}
                ...
                return (
                  ...
                  >
                    <Link to={`/career/${career.id}`}>
                      <div className="bg-pw-surface border border-white/8 rounded-2xl p-5 hover:border-pw-white/20 hover:bg-pw-surface2 transition-all group cursor-pointer h-full flex flex-col justify-between">
                        <div>
                          {/* Icon + Field Badge */}
      ...
      {activeTab === 'jobs' && (
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <button
              onClick={() => setJobField('')}
              className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${!jobField ? 'border-pw-blue bg-pw-blue/10 text-pw-blue' : 'border-pw-white/15 bg-pw-white/5 text-pw-gray/90 hover:border-pw-blue hover:text-pw-blue'}`}
            >
              All Fields
            </button>
            {fields.map(f => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${jobField === f ? 'border-pw-blue bg-pw-blue/10 text-pw-blue' : 'border-pw-white/15 bg-pw-white/5 text-pw-gray/90 hover:border-pw-blue hover:text-pw-blue'}`}
              >
                {f}
              </button>
            ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {jobs.map((job, i) => (
                <motion.a
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.04 }}
                  className="group bg-pw-surface border border-white/8 rounded-2xl p-5 hover:border-pw-blue/30 hover:bg-pw-surface2 transition-all cursor-pointer h-full flex flex-col justify-between"
                >
                  <div>
