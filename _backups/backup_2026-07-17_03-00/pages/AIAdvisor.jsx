...
const AIAdvisor = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-pw-black relative">
      ...
      {/* Left Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-40 w-72 bg-pw-surface border-r border-pw-white/5 flex flex-col transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Header */}
        <div className="p-5 border-b border-pw-white/5 flex items-center justify-between">
          ...
          <button 
            onClick={() => setIsSidebarOpen(false)} 
            className="md:hidden text-pw-gray hover:text-pw-white p-2 rounded-lg hover:bg-pw-white/5"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-pw-black">
        {/* Chat Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-pw-white/5 bg-pw-surface/30 backdrop-blur-md z-10">
          <div className="flex items-center gap-3">
            <button 
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-pw-muted text-[10px]">Ready to advise based on RIASEC &amp; SCCT theories</span>
              </div>
            </div>
          </div>
          <button 
            onClick={exportChat} 
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-pw-blue/5 text-pw-blue border border-pw-blue/10 hover:bg-pw-blue/15 transition-all rounded-xl"
            title="Export Advice"
          >
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          {messages.length <= 1 && (
            <div className="max-w-2xl mx-auto py-8 text-center flex flex-col items-center justify-center">
              <motion.div 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full text-left">
                {[
                  ...
                  >
                    <span className="text-pw-white font-bold text-sm block mb-1 group-hover:text-pw-blue transition-colors">{act.title}</span>
                    <span className="text-pw-gray text-xs leading-relaxed">{act.desc}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`max-w-[80%] md:max-w-xl w-fit group relative ${isUser ? 'order-first' : ''}`}>
                      {isUser ? (
                        ...
                        <div className="space-y-3">
                          <div className="px-5 py-4 rounded-[24px] rounded-tl-[4px] bg-pw-surface border border-pw-white/5 text-pw-gray text-sm leading-relaxed whitespace-pre-line relative">
                            ...
                            <div className="flex items-center justify-end gap-2 mt-4 pt-2.5 border-t border-pw-white/5 opacity-40 group-hover:opacity-100 transition-opacity">
                              <button
                                ...
                                className={`p-1.5 rounded-lg hover:bg-pw-white/5 transition-all ${speakingMsgIdx === i ? 'text-pw-blue' : 'text-pw-gray'}`}
        ...
        <div className="px-6 pb-6">
          <div className="flex items-center gap-3 bg-pw-surface border border-pw-white/10 rounded-full pl-5 pr-2 py-2 focus-within:border-pw-blue/40 transition-all shadow-lg">
            <button 
              onClick={() => {
            ...
            <textarea
              ...
              placeholder="Ask anything about your career or DELSU courses..."
              rows={1}
              className="flex-1 bg-transparent text-pw-white placeholder-pw-muted resize-none focus:outline-none text-sm py-2 max-h-32"
